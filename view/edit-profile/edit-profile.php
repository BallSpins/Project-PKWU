...
// === Ambil data murid & user dari MySQL (MYSQLi) ===
$sql = "
    SELECT u.username, u.email, u.profile_pic, 
           m.nama_lengkap, m.kelas, m.jurusan, m.nisn
    FROM users u
  ...
  <section class="flex-1 bg-white shadow-lg m-4 md:m-6 rounded-lg p-6 sm:p-8">
    ...
    <form action="../../backend/update-profile.php" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div>
        ...
        <input type="email" name="email" value="<?= htmlspecialchars($email) ?>" class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm p-2">
      </div>
      ...
      <div class="col-span-2">
        ...
        <input type="text" name="nisn" value="<?= htmlspecialchars($nisn) ?>" class="mt-1 block w-full md:w-1/2 border-gray-300 rounded-lg shadow-sm p-2">
      </div>
